...
ax = sns.barplot(df, x='season', y=filter_penyewa(), estimator='sum')
...
ax = sns.barplot(df, x='weathersit', y=filter_penyewa(), estimator='sum')
ax.bar_label(ax.containers[0], fontsize=10)
plt.xlabel('Cuaca')
plt.ticklabel_format(style='plain', axis='y')
...
ax = sns.barplot(df, x='temp category', y=filter_penyewa(), estimator='sum')
ax.bar_label(ax.containers[0], fontsize=10)
